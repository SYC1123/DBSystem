...
public class ChangeInfoNetWorkHelper {
    private static final int HANDLER_MSG_TELL_RECV = 1;

    private ChangeInfoCallback callback;

    private AppCompatActivity mContent;

    public ChangeInfoNetWorkHelper(AppCompatActivity appCompatActivity) {
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            if (msg.obj.equals("1")) {
                callback.onSucceed("信息修改成功！");
            } else {
                callback.onFalied(msg.obj.toString());
            }
        }
    };
    public void startNetThread(final String host, final int port, final String data, ChangeInfoCallback myCallback) {
